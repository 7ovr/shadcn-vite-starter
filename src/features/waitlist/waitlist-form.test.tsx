...
describe("waitlist form", () => {
  ...
  it("adds a valid signup to the list and clears the form", async () => {
    ...
    const list = screen.getByRole("list", { name: "Waitlist" })
    expect(await within(list).findByText("Grace Hopper")).toBeInTheDocument()
  ...
  it("shows the API error for a duplicate email", async () => {
    ...
    expect(
      await screen.findByText("That email is already on the waitlist.")
    ).toBeInTheDocument()
  })
})
