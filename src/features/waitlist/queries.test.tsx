...
import { useJoinWaitlist, waitlistQueryOptions } from "./queries"
...
describe("waitlist queries", () => {
  ...
  it("is filled by the route loader before the page renders", async () => {
    ...
    expect(
      queryClient.getQueryData(waitlistQueryOptions.queryKey)
    ).toHaveLength(2)
  })
  ...
  it("adds an entry and invalidates the cached list", async () => {
    ...
    expect(
      queryClient.getQueryState(waitlistQueryOptions.queryKey)?.isInvalidated
    ).toBe(true)
    const entries = await queryClient.fetchQuery(waitlistQueryOptions)
